...
| RESIL-02 | NEVER retry on `context.Canceled` / `context.DeadlineExceeded` | unit | `go test -run TestRetry_NeverRetriesCtxErrors -race -count=1` | ❌ Wave 0: `retry_test.go` |
...
| RESIL-03 | `computeBackoff` returns `max(jitter, retryAfter)` capped at `maxRetryWait` | unit | `go test -run TestComputeBackoff_HonorsRetryAfter -race -count=1` | ❌ Wave 0: same file |
...
| RESIL-07 | `WithCache(ttl <= 0)` disables cache | unit | `go test -run TestWithCache_NonPositiveTTLDisables -race -count=1` | ❌ Wave 0: `options_test.go` extension OR `cache_test.go` |
