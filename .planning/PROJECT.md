...
| CL-17: Narrow Gold-Rule-3 exception — `TestValidators_NoSensitiveData` in `validate_test.go` is permitted ALONGSIDE the per-validator `TestValidateCountry`/`TestValidateLanguage`/`TestValidateDateRange` because it asserts a CROSS-CUTTING invariant (ERR-04: validator error messages must contain ONLY caller-supplied input, never HTTP/transport metadata like `"http"`, `"://"`, `"Body:"`, `"Authorization"`). The invariant spans every validator's error path; folding it into each per-validator test would duplicate the assertion three times and dilute the "the rule is one property of the whole validator surface" framing. Scope: THIS test only — future cross-cutting tests must be argued for individually and recorded here. Similar in shape to CL-14 (a second top-level TestXxx tied to a specific cross-cutting requirement). The test file `validate_test.go:275-333` carries a `// Key Decision CL-17` comment pointing back to this entry. | Gold Rule 3 ("one TestXxx per exported production function") has no documented exception mechanism in CLAUDE.md or PROJECT.md, so without this entry future maintainers reading `validate_test.go` would reasonably believe the rule is loose. Recording the narrow exception keeps Gold Rule 3 enforceable for every other test by making the one allowed exception explicit, named, and scope-limited. Cross-cutting security audits (no-leak invariants, no-init invariants, no-global-state invariants) are the recognized exception category — `TestNoInitOrGlobalState` in `internal_test.go` is the structural sibling that audits ALL production files at once. Source: `.planning/phases/01-foundation/01-REVIEW.md` WR-02. | Adopted in Phase 1 (gap closure) |
| **CL-18: Three Phase 5 mechanical clarifications recorded as a single Key Decision: (a) action-version policy overriding CONTEXT.md values — codecov/codecov-action with use_oidc: true for token-free OIDC on public repos, actions/attest-build-provenance@v4 (NOT @v1; v1/v2/v3 are deprecated per RESEARCH Pitfall 2), plus actions/checkout, actions/setup-go, goreleaser/goreleaser-action, golangci/golangci-lint-action@v7, golang/govulncheck-action@v1. Dependabot (.github/dependabot.yml) is the authoritative source for current pin majors — current state at this entry's last update: checkout@v6, setup-go@v6, goreleaser-action@v7, codecov-action@v6 — but this policy entry is intentionally written at the action-set level, not the version level, because Dependabot will keep advancing the majors and rewriting CL-18 on every bump is churn; (b) the ROADMAP Phase 5 SC#3 "< 5 ms cached" perf budget is reinterpreted to apply to Countries/Languages/Subdivisions, NOT to PublicHolidays, because RESIL-07 / Phase 4 D-83 explicitly excludes holiday endpoints from caching — BenchmarkClient_Countries is the canonical cached-path measurement, NOT a "cached" sub-benchmark of PublicHolidays; (c) Phase 5 tests use context.Background() + context.WithTimeout, NOT t.Context() — Pitfall 1: t.Context() is Go 1.24+ and would break the Go 1.23 CI matrix leg.** | (a) CONTEXT.md was authored before the action versions drifted; pinning to the stale values would silently break (attest @v1 deprecated, codecov @v4 missing OIDC). Source-current pinning verified via RESEARCH WebFetch of each action's README 2026-05-28. (b) ROADMAP SC#3's literal "< 5 ms cached PublicHolidays" cannot be measured because the cache layer never accelerates PublicHolidays by design. Forcing the budget against PublicHolidays would either produce a meaningless "cached" measurement (cache no-op, equal to cold) or violate RESIL-07's stale-data policy. (c) The CI matrix lower bound is Go 1.23; using a Go 1.24+ test helper makes the matrix red despite identical functional behavior. Anchored on .planning/phases/05-distribution/05-RESEARCH.md §"Assumptions Log" A2/A3/A4 + §"Common Pitfalls → Pitfalls 1, 2, 5" + §"User Constraints → Source-current notes". | Adopted in Phase 5 |
| **CL-19: Supersedes CL-18 part (c). Go floor bumped from 1.23 → 1.24 and tests migrated to t.Context() (commit 2123f97, 2026-05-29). CI matrix is now ["1.24.x", "1.25.x", "stable"]. Companion lint additions in the same time window: bodyclose + noctx (request hygiene), tparallel (subtests under parallel parents must also be parallel), godox (TODO/FIXME visibility), godoclint with default: basic + require-stdlib-doclink + no-unused-link (godoc completeness + clickable [pkg.Type] doclinks for pkg.go.dev rendering). The `usetesting` linter family (which would have flagged context.Background() in tests) was the trigger for the migration — keeping CL-18 part (c) in force would have produced ~20 lint findings the moment the floor moved.** | Go 1.23 reached end of mainline support; the iter.Seq feature that motivated the 1.23 floor in PROJECT.md remains available in 1.24+ (additive, not deprecated). t.Context() (Go 1.24+) gives subtests automatic cancellation when t.Cleanup runs, eliminating boilerplate ctx-cancel goroutines in 14 test files. The lint additions are mechanical hygiene catching pre-existing real issues (e.g., bodyclose surfaced missed defer-close paths in error branches; tparallel surfaced 3 subtests that inherited parent t.Parallel without claiming their own). Anchored on commit 2123f97 + PR #13 (fix/release-workflow-dispatch, ci-lint scope expansion) + PR #14 (chore/godoc-cleanup, godoclint enablement + ~127 doclink fixes + single-pkg-doc cleanup). | Adopted in Phase 5 (post-planning revision) |
