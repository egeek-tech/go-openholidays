...
A production function may carry a `// audit:ok YYYY-MM-DD` line directly above it (separated from the godoc by a blank line so it never renders on pkg.go.dev), certifying its logic was reviewed correct on that date. **Any change to the function's code or behavior MUST delete its `audit:ok` line in the same commit** — the function must be re-audited before re-marking. Pure doc-comment / typo fixes are exempt. The mark is a freshness signal; a retained mark on changed code falsely certifies logic never reviewed in its current form. Enforced by convention + code review today (a CI / pre-commit guard is a future follow-up). Full text and rationale: `.planning/codebase/CONVENTIONS.md`.
