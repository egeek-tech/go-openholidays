...
A production function may carry a `// audit:ok YYYY-MM-DD` line directly above it (separated from the godoc by a blank line so it never renders on pkg.go.dev), certifying its logic was reviewed correct on that date. **Any change to the function's code or behavior MUST delete its `audit:ok` line in the same commit** — the function must be re-audited before re-marking. Pure doc-comment / typo fixes are exempt. The mark is a freshness signal; a retained mark on changed code falsely certifies logic never reviewed in its current form. Enforced by convention + code review today (a CI / pre-commit guard is a future follow-up). The same mark and invalidation rule also cover test functions (`TestXxx`, `Example*`, `Fuzz*`) in `*_test.go` — there the mark certifies the assertions encode the *correct* expected behavior, not merely that the test passes. Full text and rationale: `.planning/codebase/CONVENTIONS.md`.

### Rule 6 — Commit type must match the change; a fix is committed as `fix:`

Every commit's Conventional-Commit type states what the change actually *is*, and its subject names what the change addresses. A change that corrects a defect — wrong runtime behavior **or** wrong public documentation / godoc that would mislead a consumer — is committed as `fix(scope): <what it fixes>`, never buried inside a `chore:` / `refactor:` / `test:` commit; a new capability is `feat:`. **Never downgrade a real fix or feature to a non-releasing type to dodge cutting a release, and never inflate routine work into `fix:` / `feat:`** — selecting a type to manipulate release behavior corrupts the changelog, the release signal, and the audit trail. Work that genuinely has no consumer-facing effect and fixes no defect (internal tooling, planning docs, tests, pure refactors) correctly uses the non-releasing types. Which types cut a release in this repo is defined by `release-please-config.json` (currently `feat` / `fix` / `perf` / `deps` / `docs`) — read it, never assume. Full text and rationale: `.planning/codebase/CONVENTIONS.md`.
